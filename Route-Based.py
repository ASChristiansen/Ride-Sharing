...
for k in K:
    for r in feasible_routes[k]:
        r_tuple = tuple(r)
        y[k, r_tuple] = model.addVar(vtype=GRB.BINARY, name=f"y_{k}_{'_'.join(r_tuple) if r else 'stay'}")
...
if model.SolCount > 0:
    ...
    print(f"Best objective: {best_obj:.2f}, Best bound: {best_bound:.2f}, GAP: {gap:.2f}%")
...
if model.status == GRB.OPTIMAL:
    ...
    print(f"Optimal solution found (tolerance 1.00e-04)")
    print(f"Best objective {best_obj:.12e}, best bound {best_bound:.12e}, gap {gap:.4f}%")
    print("Optimal solution found!")

    total_cost = sum(cost(k, list(r), C, d) * y[k, r].X for (k, r) in y)
    total_rev = sum(route_revenue[k, r] * y[k, r].X for (k, r) in y)
    print(f"Total cost: {total_cost:.2f}, revenue: {total_rev:.2f}, objective: {total_cost - total_rev:.2f}")

    # ✅ Collect selected routes (y[k, r] == 1)
    for (k, r), var in y.items():
        if var.X > 0.5:
            route_list = [k] + list(r)
            route_str = " → ".join(route_list)
